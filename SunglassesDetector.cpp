...
void startDetection()
{
	...
	deserialize("sunglasses_detector.svm") >> detector;

	int cnt = 0, cntTillTen = 0;
	bool detected = false, checkForCntTillTen = false, writeTheText = false;
	...
	do
	{
		...
		if (!temp.empty())
		{
			cv_image<bgr_pixel> dlib_frame(temp);
			win.set_image(dlib_frame);

			std::vector<dlib::rectangle> dets = detector(dlib_frame);
			win.clear_overlay();
			win.set_image(dlib_frame);
			win.add_overlay(dets, rgb_pixel(255, 0, 0));
			...
			if (dets.size() > 0) {
				cnt++;
				checkForCntTillTen = 1;
			}

			if (checkForCntTillTen) {
				cntTillTen++;
			}
